...
function bindListeners() {
    ...
    // 3. pH Level
    const phRef = ref(db, 'iot/ph');
    onValue(phRef, (snapshot) => {
        const val = snapshot.val() !== null ? snapshot.val() : 0;
        document.getElementById('val-ph').textContent = Number(val).toFixed(1);
        updateSyncTime();
    });

    // 4. TDS Level
    const tdsRef = ref(db, 'iot/tds');
    onValue(tdsRef, (snapshot) => {
        const val = snapshot.val() !== null ? snapshot.val() : 0;
        document.getElementById('val-tds').textContent = val;
        updateSyncTime();
    });

    // 5. Temperature
    const tempRef = ref(db, 'iot/temperature');
    onValue(tempRef, (snapshot) => {
        const val = snapshot.val() !== null ? snapshot.val() : 0;
        document.getElementById('val-temp').textContent = Number(val).toFixed(1);
        updateSyncTime();
    });

    // 6. Humidity
    const humidityRef = ref(db, 'iot/humidity');
    onValue(humidityRef, (snapshot) => {
        const val = snapshot.val() !== null ? snapshot.val() : 0;
        document.getElementById('val-humidity').textContent = val;
        updateSyncTime();
    });

    // 7. RFID Log
    const rfidRef = ref(db, 'iot/rfid/log');
    onValue(rfidRef, (snapshot) => {
        const tbody = document.getElementById('logs-tbody');
        const data = snapshot.val();

        if (!data) {
            tbody.innerHTML = `<tr><td colspan="4" style="text-align: center; color: var(--text-secondary);">No entry records.</td></tr>`;
            return;
        }

        // Convert entries object to sorted list
        const logs = [];
        for (let key in data) {
        ...
        tbody.innerHTML = '';
        // Slice to display the latest 10 logs max
        logs.slice(0, 10).forEach(log => {
            const tr = document.createElement('tr');
            tr.innerHTML = `
                <td style="font-weight: 600;">${escapeHTML(log.nama)}</td>
                <td style="font-family: monospace;">${escapeHTML(log.uid)}</td>
                <td>${escapeHTML(log.waktu_server)}</td>
                <td><span class="status-badge-inline ${escapeHTML(log.status)}">${escapeHTML(log.status)}</span></td>
            `;
